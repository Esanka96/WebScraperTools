...
async def get_cf_clearance(
    ...
    *,
    ...
    """
    Retrieve Cloudflare cf_clearance cookie for a given URL.
    ...
    timeout : float, optional
        Timeout for solving challenge (default: 30s)
    proxy : Optional[str], optional
        Proxy to use (e.g., http://user:pass@host:port)
    user_agent : Optional[str], optional
        Custom user-agent; random Chrome UA if None.
    headed : bool, optional
        Show browser window if True.
    disable_http2 : bool, optional
        Disable HTTP/2.
    disable_http3 : bool, optional
        Disable HTTP/3.

    Returns
    -------
    Optional[str]
        The cf_clearance cookie value, or None if not solved.
    """
    user_agent = user_agent or get_chrome_user_agent()
    ...
    async with CloudflareSolver(
        ...
        return clearance_cookie["value"] if clearance_cookie else None
...
async def get_cf_clearance_info(url: str, *, timeout: float = 30.0, proxy: Optional[str] = None, user_agent: Optional[str] = None, headed: bool = False, disable_http2: bool = False, disable_http3: bool = False) -> Optional[dict]:
    """
    Minimal wrapper to return the Cloudflare clearance info as a dict (like json_data).
    """
    ua = user_agent or get_chrome_user_agent()
    async with CloudflareSolver(
        user_agent=ua,
        timeout=timeout,
        http2=not disable_http2,
...
def get_cf_clearance_info_sync(url: str, **kwargs):
    import asyncio
    return asyncio.run(get_cf_clearance_info(url, **kwargs))
