...
while p < len(url_list):
    try:
        ...
        try:
            uc_soup = first_drive(url,request_cookies)
        ...
        try:
            All_articles = first_drive("https://journals.aai.org/" + soup.find('div', class_='view-current-issue').find('a').get('href'),request_cookies).find('div',class_='section-container').findAll('div', class_='al-article-item-wrap al-normal')
        except:
            All_articles = use_drive(
                "https://journals.aai.org/" + soup.find('div', class_='view-current-issue').find('a').get('href'),
        ...
        while i < len(All_articles):
            ...
            try:
                ...
                try:
                    Article_details = first_drive(
                        "https://journals.aai.org" + All_articles[i].find('h5', class_='item-title').find('a').get('href'),request_cookies)
                except:
                    Article_details = use_drive(
                        "https://journals.aai.org" + All_articles[i].find('h5', class_='item-title').find('a').get(
                            'href'), request_cookies)
