...
def get_current_issue(list):
    ...
    publication_statuses = ['Preprint','Advance publication','Issue in print','Issue in progress','A head of print','Early Access',
        'Internet prepublication','Prepublication','Online First issue','Early view',"Early Views",'Advance Publishing','Advance Access','Ahead of Print',
...
while url_index < len(url_list):
    try:
        ...
        issue_link='https://muse.jhu.edu'+get_soup(url).find('div',{'id':'available_issues_list_text'}).findAll('div',class_="vol_group")[get_current_issue(All_issue_list)].find('a').get('href')
        ...
        while article_index < len(all_articles):
            ...
            try:
                Article_name=all_articles[article_index].find('div', class_='title').find('a').text.strip()
                Article_link = 'https://muse.jhu.edu'+all_articles[article_index].find('div', class_='title').find('a').get('href')

                Page_range = all_articles[article_index].find('div', class_='pg').text.strip().rsplit('p. ', 1)[-1].rstrip('.') if all_articles[article_index].find(
                    'div', class_='pg') else ""
                DOI = all_articles[article_index].find('div', class_='doi').text.strip().rsplit('doi.org/', 1)[-1].rstrip('.') if all_articles[article_index].find(
                    'div', class_='doi') else ""
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    try:
                        ...
                        time.sleep(2)
                        raise Exception(f"Corrupted PDF file")
